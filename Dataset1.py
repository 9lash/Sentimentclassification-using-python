...
if __name__ == '__main__':
    ...
    try:
        ...
        fname_trn1 = os.path.join(args.d, "dt1_trn_pca.svm") 
        fname_trn2 = os.path.join(args.d, "dt2_trn_pca.svm") 
        fname_trn3 = os.path.join(args.d, "dt3.trn.svm") 
        #fname_trn4 = os.path.join(args.d, "dt1_vld_pca.svm") 
        #fname_trn5 = os.path.join(args.d, "dt3.vld.svm") 




        fname_vld = os.path.join(args.d, "dt2_vld_pca.svm")
        fname_tst = os.path.join(args.d, "dt2_tst_pca.svm")

        fname_vld_lbl = os.path.join(args.d, "dt%d.%s.pca.lbl" % (args.id, "vld"))
        fname_tst_lbl = os.path.join(args.d, "dt%d.%s.pca.lbl" % (args.id, "tst"))

        fname_vld_pred = os.path.join(args.d, "dt%d.%s.pca.pred" % (args.id, "vld"))
        fname_tst_pred = os.path.join(args.d, "dt%d.%s.pca.pred" % (args.id, "tst"))
        
       
        
        ### reading labels
        from sklearn.datasets import dump_svmlight_file, load_svmlight_file
        data_trn1, lbl_trn1 = load_svmlight_file(fname_trn1, n_features=100, zero_based=True)
        data_trn2, lbl_trn2 = load_svmlight_file(fname_trn2, n_features=100, zero_based=True)
        data_trn3, lbl_trn3 = load_svmlight_file(fname_trn3, n_features=100, zero_based=True)
        ...
        data_trn = scipy.sparse.vstack( (data_trn1, data_trn2, data_trn3, data_trn4, data_trn5) )

        lbl_trn = np.hstack((lbl_trn1, lbl_trn2, lbl_trn3, lbl_trn4, lbl_trn5))
        #data_trn = []
        #data_trn = data_trn1
